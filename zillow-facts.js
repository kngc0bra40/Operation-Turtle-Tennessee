...
(()=>{
  ...
  function applyToProperty(record,parsed,{decisions={}}={}){
    ...
    const derivedProfile=(field,value,source='zillow')=>{const old=profile[field];if(precedence?.sameValue?.(old,value)||String(old??'')===String(value??''))return;const placeholder=neutralPlaceholder(old),oldSource=sources[field]||((precedence?.isBlank(old)||placeholder)?'inferred':'user-confirmed'),decision=precedence?.canReplace(placeholder?null:old,oldSource,value,source)||{allowed:old===undefined||old===null||placeholder,matching:String(old)===String(value),source};if(decision.matching)return;if(decision.allowed){profile[field]=value;sources[field]=decision.source||source;updated.push(field)}};
    ...
    if(Number(next.sqft)>0&&!(Number(profile.residenceSquareFootage)>0)){profile.residenceSquareFootage=Number(next.sqft);sources.residenceSquareFootage=sources.sqft||'zillow';updated.push('residenceSquareFootage')}
    if(['livable','likely_livable','condition_unknown','non_livable'].includes(residenceStatus)){derivedProfile('existingResidencePresent',true,statusSource);if(residenceStatus==='livable')derivedProfile('existingResidenceLivable',true,statusSource);else if(residenceStatus==='non_livable')derivedProfile('existingResidenceLivable',false,'zillow');else if(profile.existingResidenceLivable===undefined){profile.existingResidenceLivable=null;sources.existingResidenceLivable='zillow';updated.push('existingResidenceLivable')}if(['likely_livable','condition_unknown','non_livable'].includes(residenceStatus))derivedProfile('residenceCondition','unknown-condition','zillow');profile.residenceLivability=residenceStatus==='likely_livable'?'likely-needs-review':residenceStatus;profile.residenceDataSource=statusSource==='user-confirmed'?'User':'Zillow';profile.residenceFactsUsed=homeSignals}
