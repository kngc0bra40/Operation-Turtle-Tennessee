...
(()=>{
  'use strict';
  const VERSION='2.2.0',precedence=window.OTSourcePrecedence;
  ...
  function parse(text=''){
    ...
    if(publicWater){const line=publicWater[0],connected=/\b(?:public water|city water)[^\n.]*(?:connected|served by|service active|in use)|(?:connected|served by|service active|in use)[^\n.]*(?:public water|city water)/i.test(line),available=/\bwater available\b/i.test(raw),sourceOnly=/water\s*:\s*public/i.test(line),state=connected?'connected':available?'available':'recorded',waterSource=connected||sourceOnly&&!available?'public-water':state;add('publicWater',state,line,'medium',connected?'':'Connection is not confirmed.');add('waterSource',waterSource,line,'medium',connected?'':'Availability is not a confirmed connection.')}
  ...
  function deriveResidenceStatus(record={},rawText=''){
    const profile=record.propertyIntelligence?.propertyProfile||{},type=`${record.homeType||''} ${record.propertySubtype||''} ${record.propertyType||''}`.toLowerCase(),raw=String(rawText||'').toLowerCase();
    if(profile.existingResidenceLivable===true||profile.residenceCondition==='livable')return 'livable';
    if(/uninhabitable|not habitable|tear.?down|condemned/.test(raw)||profile.residenceCondition==='major-rehabilitation'&&profile.existingResidenceLivable===false)return 'non_livable';
    const residential=/single.?family|residence|residential|house|home|cabin|manufactured|mobile home|farmhouse/.test(type),homeFacts=['beds','baths','fullBathrooms','sqft','yearBuilt'].filter(field=>Number(record[field])>0).length,hvac=Boolean(clean(record.heating)||clean(record.cooling)),services=['electric','waterSource','septicOrSewer','internet','driveway'].filter(field=>!neutralPlaceholder(profile[field])&&profile[field]&&profile[field]!=='none').length;
    if(residential&&homeFacts>=2&&(hvac||services>=2))return 'likely_livable';
    if(residential||homeFacts>=2||profile.existingResidencePresent===true)return 'condition_unknown';
    if(profile.existingResidencePresent===false||(/raw land|vacant land|no (?:home|dwelling|residence)/.test(`${type} ${raw}`)&&!homeFacts))return 'raw_land';
    return 'unknown';
  }
  ...
  function applyToProperty(record,parsed,{decisions={}}={}){
    ...
    for(const fact of parsed?.facts||[]){
      const old=current(fact.field),placeholder=neutralPlaceholder(old),oldSource=sourceFor(sources,fact.field)||((precedence?.isBlank(old)||placeholder)?'inferred':'user-confirmed'),decision=precedence?.canReplace(placeholder?null:old,oldSource,fact.value,'zillow',{explicit:decisions[fact.field]==='zillow'})||{allowed:old===undefined||old===''||placeholder,matching:String(old)===String(fact.value),source:'zillow'};
      ...
      assign(fact.field,fact.value);recordSource(sources,fact.field,decision.source||'zillow');updated.push(fact.field);
    }
    if(!next.baths&&Number(next.fullBathrooms)>0){next.baths=Number(next.fullBathrooms);sources.baths=sources.fullBathrooms||'zillow';updated.push('baths')}
    const derivedProfile=(field,value,source='zillow')=>{const old=profile[field];if(precedence?.sameValue?.(old,value)||String(old??'')===String(value??''))return;const placeholder=neutralPlaceholder(old),oldSource=sourceFor(sources,field)||((precedence?.isBlank(old)||placeholder)?'inferred':'user-confirmed'),decision=precedence?.canReplace(placeholder?null:old,oldSource,value,source)||{allowed:old===undefined||old===null||placeholder,matching:String(old)===String(value),source};if(decision.matching)return;if(decision.allowed){profile[field]=value;recordSource(sources,field,decision.source||source);updated.push(field)}};
    const residenceStatus=deriveResidenceStatus({...next,propertyIntelligence:{...(next.propertyIntelligence||{}),propertyProfile:profile}},parsed?.rawText),statusSource=residenceStatus==='livable'?'user-confirmed':'zillow';
    derivedProfile('residenceStatus',residenceStatus,statusSource);
    const homeSignals=['beds','baths','fullBathrooms','sqft','structureArea','yearBuilt','homeType'].filter(field=>parsed?.facts?.some(fact=>fact.field===field));
    if(Number(next.sqft)>0&&!(Number(profile.residenceSquareFootage)>0)){profile.residenceSquareFootage=Number(next.sqft);recordSource(sources,'residenceSquareFootage',sources.sqft||'zillow');updated.push('residenceSquareFootage')}
    if(['livable','likely_livable','condition_unknown','non_livable'].includes(residenceStatus)){derivedProfile('existingResidencePresent',true,statusSource);if(residenceStatus==='livable')derivedProfile('existingResidenceLivable',true,statusSource);else if(residenceStatus==='non_livable')derivedProfile('existingResidenceLivable',false,'zillow');else if(profile.existingResidenceLivable===undefined){profile.existingResidenceLivable=null;recordSource(sources,'existingResidenceLivable','zillow');updated.push('existingResidenceLivable')}if(['likely_livable','condition_unknown','non_livable'].includes(residenceStatus))derivedProfile('residenceCondition','unknown-condition','zillow');profile.residenceLivability=residenceStatus==='likely_livable'?'likely-needs-review':residenceStatus;profile.residenceDataSource=statusSource==='user-confirmed'?'User':'Zillow';profile.residenceFactsUsed=homeSignals}
    else if(residenceStatus==='raw_land'){derivedProfile('existingResidencePresent',false,'zillow');derivedProfile('existingResidenceLivable',false,'zillow')}
    ...
    const existingSite={...(next.site||{})},setSite=(key,value,sourceField)=>{if(!value||!precedence?.isBlank(existingSite[key])&&!/^(?:tbd|unknown|not recorded)$/i.test(String(existingSite[key])))return;existingSite[key]=value;sources[`site.${key}`]=sourceFor(sources,sourceField)||'zillow';updated.push(`site.${key}`)};
    ...
    next.site=existingSite;next.development={...(next.development||{})};const setDevelopment=(key,value,sourceField)=>{if(!value||!['',null,undefined,'unknown','tbd'].includes(next.development[key]))return;next.development[key]=value;sources[`development.${key}`]=sourceFor(sources,sourceField)||'zillow';updated.push(`development.${key}`)};if(profile.existingResidencePresent)setDevelopment('homesite','home','residenceStatus');if(['year-round','verified','recorded'].includes(profile.driveway))setDevelopment('driveway','existing','driveway');const installedUtilities=['verified','verified-well','public-water'].some(value=>[profile.electric,profile.waterSource,profile.septicOrSewer].includes(value));if(installedUtilities)setDevelopment('utilities','onsite','electric');else if(['available','recorded'].some(value=>[profile.electric,profile.waterSource,profile.septicOrSewer].includes(value)))setDevelopment('utilities','roadside','electric');
    next.infrastructure={...(next.infrastructure||{}),electric:profile.electric==='verified'?'onsite':profile.electric||next.infrastructure?.electric,well:profile.waterSource==='verified-well'?'existing':profile.waterSource||next.infrastructure?.well,septic:profile.septicOrSewer==='verified'?'existing':profile.septicOrSewer||next.infrastructure?.septic,driveway:['year-round','verified','recorded'].includes(profile.driveway)?'existing':profile.driveway||next.infrastructure?.driveway,internet:profile.internet||next.infrastructure?.internet,cable:profile.cable||next.infrastructure?.cable,parking:Number(next.parkingSpaces)>0?'recorded':next.infrastructure?.parking,structures:structures.length?'recorded':next.infrastructure?.structures};
  ...
  function buildReview(property,conflicts=[]){
    ...
    if(['likely_livable','condition_unknown'].includes(profile.residenceStatus)||profile.existingResidencePresent&&profile.existingResidenceLivable!==true&&!['non_livable','raw_land'].includes(profile.residenceStatus))add('livability','Confirm whether the residence is currently livable.','Listing facts establish a home, not its condition.','Existing Home & Infrastructure');
  ...
  function runRegressionChecks(){
    ...
    const checks={fixtureCoverage:['acres','beds','fullBathrooms','baths','sqft','structureArea','yearBuilt','homeType','heating','cooling','roof','parkingSpaces','carport','driveway','deck','coveredPorch','fencing','mountainView','creek','levelLand','private','slopedLand','wooded','barn','stable','storageBuilding','septicOrSewer','waterSource','publicWater','electric','internet','cable','hoaStatus','parcelNumber','annualTaxes','assessedValue'].every(field=>fields.has(field)),landLevelDoesNotPopulateHouseLevels:!fields.has('levels'),mappedHome:mapped.property.propertyIntelligence.propertyProfile.existingResidencePresent===true&&mapped.property.propertyIntelligence.propertyProfile.existingResidenceLivable===null&&mapped.property.propertyIntelligence.propertyProfile.residenceStatus==='likely_livable'&&mapped.property.propertyType==='existing-home-renovation',livingAreaCanonical:mapped.property.sqft===1152&&mapped.property.propertyIntelligence.propertyProfile.residenceSquareFootage===1152,mappedLand:mapped.property.propertyIntelligence.propertyProfile.woodedOpenMix==='mixed'&&mapped.property.propertyIntelligence.propertyProfile.slopeCharacter==='mixed-moderate',mappedWater:mapped.property.propertyIntelligence.propertyProfile.waterFeatureType==='creek'&&mapped.property.propertyIntelligence.propertyProfile.waterSource==='available',publicWaterSourceNormalized:applyToProperty({id:'water',lat:35,lng:-84,propertyIntelligence:{propertyProfile:{}}},parse('Water: Public')).property.propertyIntelligence.propertyProfile.waterSource==='public-water',parcelMapped:mapped.property.parcel?.parcelId==='077 04200 000',developmentAssumptions:mapped.property.development.homesite==='home'&&mapped.property.development.driveway==='existing'&&mapped.property.development.utilities==='onsite'&&/available/.test(mapped.property.site.Well)&&/inspect/.test(mapped.property.site.Septic),costResponds:mapped.property.developmentCost.septic==='existing-unverified',structuresMapped:['carport','barn','stable','storageBuilding'].every(field=>mapped.property.propertyIntelligence.zillowLand[field]==='yes'),manualValueProtected:protectedMap.property.acres===40&&protectedMap.conflicts.some(conflict=>conflict.field==='acres'),rejectedCandidateStored:protectedMap.property.sourceCandidates.acres.length===1,neutralPlaceholdersFill:applyToProperty({id:'neutral',lat:35,lng:-84,propertyIntelligence:{propertyProfile:{electric:'unknown',waterSource:'unknown',septicOrSewer:'unknown'}}},parse('Electric connected\nSeptic Tank\nWater: Public')).property.propertyIntelligence.propertyProfile.electric==='verified',reviewsFocused:mapped.reviewItems.length<=3,noUnmappedUserWarnings:mapped.reviewItems.every(item=>!/mapped|unsupported|parser/i.test(item.issue)),idempotent:applyToProperty(mapped.property,parsed).updated.length===0};
  ...
  window.OTZillowFacts={VERSION,PRIORITY,REGISTRY,parse,deriveResidenceStatus,applyToProperty,buildReview,runRegressionChecks};
})();
