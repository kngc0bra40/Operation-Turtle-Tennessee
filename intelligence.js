...
(function(){
  ...
  const reviewHash=value=>{let hash=2166136261;for(const char of JSON.stringify(value??null)){hash^=char.charCodeAt(0);hash=Math.imul(hash,16777619)}return (hash>>>0).toString(36)};
  function dataReview(record={}){
    const profile=profileFor(record),audit=auditImportFields(record),issues=[],resolutions=plainObject(record.reviewResolutions),add=(code,found,why,category,action='scorecard',fact=code,evidence={})=>{const id=`${String(record.id||'property')}:${code}:${fact}`,fingerprint=reviewHash(evidence);if(!issues.some(item=>item.id===id))issues.push({id,code,found,why,category,action,fact,fingerprint});};
    ...
    if(Number(record.acres)>0&&Number.isFinite(alternateAcres)&&alternateAcres>0&&Math.abs(Number(record.acres)-alternateAcres)>.2)add('acreage-conflict','Saved acreage differs from another imported acreage value.','Acreage affects land, recreation, price-per-acre, and build-site scoring.','Second-Home Build Potential','property','acres',{saved:Number(record.acres),alternate:alternateAcres,sources:{saved:record.fieldSources?.acres,alternate:record.fieldSources?.lotAcres}});
    const routeReview=window.OTRoutePolicy?.scoreLocation?.(record),missingRoutes=routeReview?.missing||(!Array.isArray(record.destinations)||!record.destinations.some(item=>Number.isFinite(Number(item?.routeMinutes??item?.durationMinutes??item?.minutes)))?['routes']:[]);
    if(missingRoutes.length)add('routes-missing','One or more property-specific routed destinations need review.','Location convenience uses routed travel times only.','Location & Convenience','routes','routes',{missing:missingRoutes,routeRevision:record.routeState?.updatedAt||'',destinations:(record.destinations||[]).map(item=>[item.type||item.category||item.label||item.name,item.routeMinutes??item.minutes,item.source])});
    if(!meaningfulValue(record.developmentCost)&&!meaningfulValue(record.sitePlanning?.costEstimates))add('costs-missing','Site-development cost information is missing.','Unknown driveway, utility, well, septic, and rehabilitation work reduces cost confidence.','Cost, Risk & Personal Fit','property','developmentCost',{developmentCost:record.developmentCost,siteCosts:record.sitePlanning?.costEstimates});
    Object.entries(plainObject(record.sourceCandidates)).forEach(([field,candidates])=>{const candidate=Array.isArray(candidates)?candidates.at(-1):null;if(candidate?.status==='rejected')add(`source-conflict-${field}`,`A ${candidate.source} value for ${field.replace(/([A-Z])/g,' $1')} differs from the saved value.`,'The suggested value was preserved without replacing the stronger saved source.','Property Details','property',field,{current:record[field],candidate:candidate.value,source:candidate.source});});
    ...
    const allIssues=issues.filter(issue=>resolutions[issue.id]?.fingerprint!==issue.fingerprint);
    return {count:allIssues.length,issues:allIssues.slice(0,3),allIssues,resolvedCount:issues.length-allIssues.length,audit};
  }
  ...
  function ruleLocationConvenience(record){
    const routed=window.OTRoutePolicy?.scoreLocation?.(record);
    if(routed){
      if(routed.score===null)return {autoScore:null,confidence:'Not enough information',reasons:[],warnings:['No verified routed travel times are recorded. Straight-line distances are not used for this score.']};
      return {autoScore:clampScore(routed.score),confidence:routed.confidence,reasons:unique(routed.reasons),warnings:unique(routed.warnings)};
    }
  ...
  function fixtureRecord(name,overrides={}){
    return {id:name,name,address:`${name} Rd`,lat:35.4,lng:-84.3,acres:32,price:360000,propertyType:'raw-land',notes:'private wooded property with trails and hunting',pros:[],cons:[],scores:{Value:60},destinations:[{type:'airport',category:'TYS airport',name:'McGhee Tyson Airport',code:'TYS',routeMinutes:58},{category:"Lowe's",name:"Lowe's",routeMinutes:30},{category:'Home Depot',name:'Home Depot',routeMinutes:35},{category:'Grocery',name:'Grocery',routeMinutes:22},{category:'Hospital',name:'Hospital',routeMinutes:34}],propertyIntelligence:{propertyProfile:{}},...overrides};
  }
  function runRegressionChecks(){
    ...
    const checks={
      ...
      ,reviewResolutionSurvivesRecalculation:recalculatedReview.count===resolvedReview.count
      ,reviewResolutionSurvivesRepeatedImport:reimportedReview.count===resolvedReview.count
      ,reviewResolutionSurvivesSameRouteRefresh:sameRouteReview.count===resolvedReview.count
      ,newConflictReopensReview:changedConflict.allIssues.some(item=>item.id===resolvedIssue.id)
      ,reviewIdentityStable:dataReview(sourceConflictRecord).allIssues.find(item=>item.code==='source-conflict-acres').id===resolvedIssue.id
