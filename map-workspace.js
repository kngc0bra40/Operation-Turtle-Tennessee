...
function install(){
 const previousOpen=window.openProperty;window.openProperty=function(id){window.OTDossierNavigation?.capture?.();activeId=String(id||'');window.OTRecreationLayers?.setActiveProperty?.(activeId);const result=previousOpen?.(id);window.OTDossierNavigation?.restore?.(activeId);syncControls();return result};
