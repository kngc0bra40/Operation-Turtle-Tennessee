...
function buildQueries(record={}){
  ...
  return [...new Set([`"${address}" previous business recreation glamping permits`,`"${address}" course campground business`,...clues.map(clue=>`"${address}" ${clue}`)])].slice(0,4);
}
