...
function subdivisionLayout(record={}){
  const status=parcelStatus(record),geometry=normalizeGeometry(record.parcelGeometry),acres=number(record.parcelIntelligence?.gisAcreage||record.acres),land=record.propertyIntelligence?.zillowLand||{},parcelCount=Math.max(record.parcelIntelligence?.parcels?.length||0,record.parcelIntelligence?.parcelIds?.length||0,geometry?.type==='MultiPolygon'?geometry.coordinates.length:geometry?1:0),existingLegalParcels=parcelCount>1,explicitNoAccess=/landlocked|no legal access/i.test(`${record.notes||''} ${record.parcel?.notes||''}`);
